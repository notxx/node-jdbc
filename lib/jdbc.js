...
JDBCConn.prototype.executeQuery = function(sql, args, callback) {
  var self = this;
  if (!callback && typeof(args) === 'function') { callback = args; args = []; }
  ...
  self._conn.prepareStatement(sql, function(err, statement) {
    ...
    if (err) {
      ...
      args.forEach(function(arg, i) { statement.setObject(i + 1, arg); });
